...
def classify_new_images_poisoned(model, new_images_dir, idx_to_class, device, save_path=None):
    ...
    plt.suptitle("New Images Classification (Poisoned Model)", y=0.95, fontsize=14)
...
def main():
    print("Starting Face Poisoned Classification v13.1.1...")
    # Set device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    # Create output directory if it doesn't exist
    output_dir = "/content/drive/MyDrive/FaceModel/Output/Poisoned_Classification"
    ...
    try:
        ...
        print(f"Loading poisoned model from: {poisoned_model_path}")
        checkpoint = torch.load(poisoned_model_path)
        ...
        # Classify new images
        classify_new_images_poisoned(
            model, 
            "/content/drive/MyDrive/FaceModel/NewPerson", 
            idx_to_class, 
            device,
            save_path=os.path.join(output_dir, 'new_images_poisoned_classification.png')
        )
